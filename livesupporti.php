	...
	else
	{
		if (isset($_POST["action"]) && $_POST["action"] == "signupin")
		{
			...
			if ($email && $password != "")
			{
				...
				$lsResponse = json_decode(postLiveSupportiRequest("https://livesupporti.com/wordpress/signupsignin", $liveSupportiData));
				if (!isset($lsResponse->wp_error) && !isset($lsResponse->errorEmail) && !isset($lsResponse->errorPassword))
				{
					...
					update_option('liveSupportiSkin', esc_html($_POST['selectLiveSupportiSkin']));
					
					if (!isset($lsResponse->error) && isset($lsResponse->code) && $lsResponse->code != "")
					{
						update_option("liveSupportiLicense", esc_html($lsResponse->code));
					}
				}
			}
			else
...
function livesupporti_uninstall_plugin() {
	if (get_option('liveSupportiEmail', false))
	{
		...
		$lsResponse = json_decode(postLiveSupportiRequest("https://livesupporti.com/wordpress/uninstall", $liveSupportiData));	
	}
